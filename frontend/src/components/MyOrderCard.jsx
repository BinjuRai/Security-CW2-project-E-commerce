...
import { Clock, Check, RefreshCw, Trash2, Package, Calendar, Tag, ChevronRight, ArrowUpRight } from "lucide-react"
import { getBackendImageUrl } from "../utils/backend-image"
      ...
      <div className="p-6 md:p-8 border-b border-[#f1d1d1]/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-[#f1d1d1] mb-1">
            ...
            <span className="text-[9px] font-bold tracking-[0.3em] uppercase">Order Reference</span>
          </div>
        ...
        <div className={`flex items-center gap-3 px-6 py-2 border rounded-full ${status.border} ${status.bg || ''}`}>
          <span className={status.textColor}>{status.icon}</span>
          <span className={`text-[10px] font-bold tracking-[0.2em] uppercase ${status.textColor}`}>
            {status.label}
          </span>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        <div className="flex items-center gap-8 border-b border-[#f1d1d1]/10 pb-6">
          <div className="flex flex-col">
            <span className="text-[9px] font-bold tracking-widest text-[#494040]/30 uppercase mb-1">Placement Date</span>
          ...
          <div className="flex flex-col">
            <span className="text-[9px] font-bold tracking-widest text-[#494040]/30 uppercase mb-1">Curation Size</span>
            <div className="flex items-center gap-2 text-[#494040] text-xs font-medium uppercase tracking-wider">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#494040]/40">Items Summary</span>
          </div>
          ...
          <div className="grid gap-6">
            {order.products?.map((item) => {
              ...
              return (
                <div key={item._id} className="flex gap-6 pb-6 border-b border-[#f1d1d1]/10 last:border-0 items-start">
                  {/* Image Frame */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-sm font-serif italic text-[#494040]">
                        NPR {formatCurrency((item.price * item.quantity) + addonsTotal)}
                      </span>
                    </div>
                    ...
                    {item.addons && item.addons.length > 0 && (
                      <div className="mt-3 bg-[#f1d1d1]/5 p-3 space-y-1 border-l border-[#f1d1d1]">
                        {item.addons.map((addon, idx) => (
                          <div key={idx} className="flex justify-between text-[10px] text-[#494040]/50 tracking-wide uppercase">
                            <span>+ {addon.name} (x{addon.quantity})</span>
                            <span>NPR {formatCurrency(addon.price * addon.quantity)}</span>
                          </div>
                        ))}
                      </div>
                    )}
        ...
        <div className="pt-8 mt-4 border-t border-[#494040] flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="flex flex-col items-start md:items-start">
            ...
            <p className="text-4xl font-serif text-[#494040]">NPR {formatCurrency(grandTotal)}</p>
          </div>
