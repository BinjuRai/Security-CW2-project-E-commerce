...
export default function CreateBannerPage() {
    ...
    const handleDelete = async (id) => {
        if (!confirm("Are you certain you wish to remove this visual asset?")) return;
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-[#f1d1d1]/30 pb-12">
                        <div className="space-y-3">
                            <div className="flex items-center gap-2 text-[#f1d1d1]">
                                ...
                                <span className="text-[10px] font-bold tracking-[0.4em] uppercase">Visual Assets</span>
                            </div>
                            <h1 className="text-5xl md:text-6xl font-serif italic tracking-tight">
                                Banner <span className="font-sans not-italic font-light">Curation</span>
                            </h1>
                            <p className="text-[11px] text-[#494040]/40 uppercase tracking-[0.1em] font-medium max-w-md">
                                Managing the aesthetic storefront visuals of the BagBelle Private Atelier.
                            </p>
                        </div>
                        <ShieldCheck className="hidden md:block w-12 h-12 text-[#f1d1d1]/30" strokeWidth={1} />
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
                    ...
                    <div className="lg:col-span-4">
                        <section className="bg-white border border-[#f1d1d1]/30 p-8 shadow-[0_10px_40px_rgba(73,64,64,0.02)]">
                            <h2 className="text-xs font-bold tracking-[0.3em] uppercase mb-8 border-b border-[#f1d1d1]/20 pb-4">New Entry</h2>
                            
                            <form onSubmit={handleSubmit} className="space-y-8">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold tracking-widest uppercase text-[#494040]/40">Nomenclature</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-4">
                                    <label className="text-[10px] font-bold tracking-widest uppercase text-[#494040]/40">Visual Asset</label>
                                    ...
                                    {imageFile && (
                                        <p className="text-[9px] text-[#494040]/60 italic text-center truncate">{imageFile.name}</p>
                                    )}
                                </div>
                                ...
                                >
                                    {loading ? (
                                        <Loader2 size={16} className="animate-spin text-[#f1d1d1]" />
                                    ) : (
                                        <>
                                            <Plus size={16} className="text-[#f1d1d1] group-hover:rotate-90 transition-transform" />
                                            <span className="text-[10px] font-bold tracking-[0.3em] uppercase">Register Asset</span>
                                        </>
                                    )}
                                </button>
                                {error && <p className="text-[9px] text-red-400 font-bold uppercase text-center mt-4">{error}</p>}
                            </form>
                        </section>
                    </div>
                    ...
                    <div className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-8">
                            <h2 className="text-xs font-bold tracking-[0.3em] uppercase text-[#494040]/60">Active Display Registry</h2>
                            <span className="text-[10px] font-serif italic text-[#f1d1d1]">{banners.length} Assets Logged</span>
                        </div>
                        ...
                        {loadingBanners ? (
                            <div className="flex flex-col items-center justify-center py-20 bg-[#f1d1d1]/5 border border-dashed border-[#f1d1d1]">
                                ...
                                <span className="text-[10px] uppercase tracking-widest opacity-40">Accessing Display Registry</span>
                            </div>
                        ) : banners.length === 0 ? (
                            <div className="py-32 text-center border border-dashed border-[#f1d1d1]">
                                <ImageIcon className="w-10 h-10 text-[#f1d1d1]/30 mx-auto mb-4" strokeWidth={1} />
                                <p className="font-serif italic text-[#494040]/40">No display assets found in the registry.</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="relative aspect-video bg-white border border-[#f1d1d1]/30 overflow-hidden shadow-sm transition-all duration-700 hover:shadow-xl hover:border-[#494040]/20">
                                            ...
                                            <div className="absolute bottom-4 left-4 right-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                                                <div className="bg-[#fffcfc] p-3 border-l-2 border-[#f1d1d1] shadow-lg">
                                                    <p className="text-[10px] font-bold tracking-widest uppercase truncate">{banner.name}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
