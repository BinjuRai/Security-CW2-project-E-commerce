...
app.use(
  express.json({
    ...
    verify: (req, res, buf, encoding) => {
      if (buf.length > 10240) {
        console.warn(
          `⚠️ Large payload rejected: ${buf.length} bytes from ${req.ip}`,
        );
      }
    },
  }),
...
app.use(
  express.urlencoded({
    ...
    verify: (req, res, buf, encoding) => {
      if (buf.length > 10240) {
        console.warn(
          `⚠️ Large URL-encoded payload rejected: ${buf.length} bytes from ${req.ip}`,
        );
      }
    },
  }),
  ...
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: ["'self'", "http://localhost:5173", "ws://localhost:5050"], 
      },
    },
...
app.use((err, req, res, next) => {
  ...
  if (err.code === "EBADCSRFTOKEN") {
    console.error(
      `❌ CSRF Token Validation Failed from ${req.ip} on ${req.path}`,
    );
